...
class CTriangle : CFigure
    ...
    {
    this.graphics = graphics; this.X = X; this.Y = Y; this.Side = Side;
}
...
protected override void Draw(Pen pen) 
{
    ...
    Point p3 = new Point(X + (int)(r * Math.Cos(Math.PI / 6)), Y + (int)(r * Math.Sin(Math.PI / 6))); Point[] triangle = { p1, p2, p3 }; graphics.DrawPolygon(pen, triangle);
}
